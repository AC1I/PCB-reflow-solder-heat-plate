...
enum menu_state_t { MENU_IDLE,
                    ...
                    MENU_INC_TEMP,
                    MENU_DEC_TEMP };
enum buttons_state_t { BUTTONS_NO_PRESS,
                       BUTTONS_BOTH_PRESS,
                       BUTTONS_UP_PRESS,
                       BUTTONS_DN_PRESS };
enum single_button_state_t { BUTTON_PRESSED,
                             BUTTON_RELEASED,
                             BUTTON_NO_ACTION };
...
// Temperature Info
const byte max_temp_array[] = { 140, 150, 160, 170, 180 };
byte       max_temp_index = 0;
#define MAX_RESISTANCE 10.0
float bed_resistance = 1.56;
#define MAX_AMPERAGE 5.0
#define PWM_VOLTAGE_SCALAR 2.0

uint8_t uPWMConstraint(2);  // Maximum duty cycle to manage current to design limits
...
void    PCBReflowOff(void);
uint8_t ConstrainCurrent(uint8_t uPWM);
void    KeyboardHandler(void);
void    PrintTemperatureMapCSV(void);
void    PrintCurTemp(void);
...
void setup() {
  ...
  analogWrite(MOSFET_PIN, MOSFET_PIN_OFF);  // VERY IMPORTANT, DONT CHANGE!
...
float       getTemp() {
  ...
  if (sensors.getDeviceCount()) {                         // The ATmega4809 has 256 bytes of EEPROM
    sensors.requestTemperatures();                        // CSV First row contains column header names
    for (int i = 0; i < sensors.getDeviceCount(); i++) {  // pwm,time,temp1, temp2, scalar
      float temp_in = sensors.getTempCByIndex(i);         // char * c = new char[N](); := calloc()
...
void PCBReflowOff(void) {
  ...
  ConstrainCurrent(MOSFET_PIN_OFF);
}

uint8_t ConstrainCurrent(uint8_t uPWM) {
  static float         maxVolts(getVolts());
  static uint8_t       constraint(uPWMConstraint);
  static elapsedMillis last(0);
  ...
  return constrain(uPWM, constraint, MOSFET_PIN_OFF - 1);
}
